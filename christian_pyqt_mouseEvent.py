...
class Window(QMainWindow):
    ...
    def mouseMoveEvent(self, event):
        if (event.buttons() & Qt.LeftButton) & self.drawing:
            ...
            if self.MODE == "rectangle":
                ...
                r = QRect(self.lastPoint, self.curPoint)
                windowPainter = QPainter(self.image)
                windowPainter.setPen(QPen(self.brushColor, self.brushSize, Qt.SolidLine,Qt.RoundCap,Qt.RoundJoin))
                windowPainter.drawRect(r)
                windowPainter.eraseRect(r)
                self.update()
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.MODE == "rectangle":
            ...
            e = QRect(self.lastPoint, self.curPoint)
            self.painter.setPen(QPen(self.brushColor, self.brushSize, Qt.SolidLine,Qt.RoundCap,Qt.RoundJoin))
            self.painter.drawEllipse(e)               

            windowPainter = QPainter(self.image)
            windowPainter.setPen(QPen(self.brushColor, self.brushSize, Qt.SolidLine,Qt.RoundCap,Qt.RoundJoin))
            windowPainter.drawEllipse(e)
            self.update()
        elif self.MODE == "line":
            l = QLine(self.lastPoint, self.curPoint)
            self.painter.setPen(QPen(self.brushColor, self.brushSize, Qt.SolidLine,Qt.RoundCap,Qt.RoundJoin))
            self.painter.drawLine(l)               

            windowPainter = QPainter(self.image)
            windowPainter.setPen(QPen(self.brushColor, self.brushSize, Qt.SolidLine,Qt.RoundCap,Qt.RoundJoin))
            windowPainter.drawLine(l)
            self.update()
